            ...
            for attempt in range(1, max_attempts + 1):
                try:
                    ...
                    if on_retry:
                        ...
                        logger.warning(
                            f"函数 {func.__name__} 第 {attempt} 次尝试失败，" f"{current_delay:.1f}秒后重试: {e}"
                        )
...
def retry_network(
    max_attempts: int = RetryConfig.TELEGRAM_RETRY, delay: float = RetryConfig.INITIAL_DELAY
) -> Callable[[Callable[..., T]], Callable[..., T]]:
    """网络请求重试装饰器
...
def retry_playwright(
    ...
    """
    return retry(
        max_attempts=max_attempts, delay=delay, exceptions=(PlaywrightTimeout, PlaywrightError)
    )
